...
const initializeSidebarProvider = (context: vscode.ExtensionContext) => {
  ...
  context.subscriptions.push(vscode.window.registerWebviewViewProvider(DOCLIN_SIDEBAR, sidebarProvider));
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(DOCLIN_VIEW_FILE_THREADS, () =>
      viewFileThreadsCommand(sidebarProvider.getWebviewView())
    )
  );

  context.subscriptions.push(
    vscode.commands.registerCommand(DOCLIN_VIEW_THREAD, (thread: Thread) =>
      viewThreadCommand(thread, sidebarProvider.getWebviewView())
    )
  );
};
...
const initializeCopilotSidebarProvider = (context: vscode.ExtensionContext) => {
  ...
  context.subscriptions.push(vscode.window.registerWebviewViewProvider(DOCLIN_COPILOT_SIDEBAR, copilotSidebar));
};
